...
def train_num(labels, im_class_num, class_sample_num, im_ratio):
    ...
    for i in range(max_class + 1):
        if i > im_class_num:                                              #only imbalance the last classes
            c_train_num.append(int(class_sample_num * im_ratio[i-im_class_num-1]))
        else:
...
def segregate(labels, c_train_num):
    ...
    c_num_mat[:,1] = 25                                 # Validation will have 25 samples per class, test will have 55 samples per class
    c_num_mat[:,2] = 55
    ...
    for i in range(num_classes):                        # Looping through all classes, to separate out the desired no. of samples
        ...
        val_idx = val_idx + c_idx[c_train_num[i]:c_train_num[i]+25]          # Add next 25 indices to val and rest 55 to test

...
class Het_Agg(nn.Module):
    ...
    def forward(self, x_list, edges_list, x_node, num_node, edge_weight_list):
        g_aggr = self.g_aggregate(x_list[0], edges_list[0], num_node, edge_weight_list[0])
        d_aggr = self.d_aggregate(x_list[1], edges_list[1], num_node, edge_weight_list[1])
        c_aggr = self.c_aggregate(x_list[2], edges_list[2], num_node, edge_weight_list[2])
        s_aggr = self.s_aggregate(x_list[3], edges_list[3], num_node, edge_weight_list[3])
...
class Het_NetEn(nn.Module):
    ...
    def forward(self, x_list, data):
        ...
        x_list[1] = self.d_het(x_list, d_edges_list, x_list[1], x_list[1].size(0), edge_weight_list)
        x_list[0] = self.g_het(x_list, g_edges_list, x_list[0], x_list[0].size(0), edge_weight_list) 
        x_list[2] = self.c_het(x_list, c_edges_list, x_list[2], x_list[2].size(0), edge_weight_list)
        x_list[3] = self.s_het(x_list, s_edges_list, x_list[3], x_list[3].size(0), edge_weight_list)
...
class Het_classify(nn.Module):
    ...
    def forward(self, x_list, edge_list, edge_weight_list):
        ...
        x = self.d_het(x_list, edge_list, x_list[1], x_list[1].size(0), edge_weight_list)
        x = torch.relu(x)
        x = F.dropout(x, self.dropout, training=self.training)
        x = self.mlp(x)

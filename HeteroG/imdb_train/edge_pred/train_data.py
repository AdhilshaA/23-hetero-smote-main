...
# A_n = 20171, P_n = 13250 , V_n = 18, embed_dim = 128, het_neigh_dim = 100
class Aminer:
    def __init__(self, args):                                  
        self.args = args
        self.cite_filename = ["p_a_train.txt", "p_p_train.txt", "p_v_train.txt"]
        self.content_filename = ["p_title_train.txt", "p_abstract_train.txt", "../aminer_edge/node_net_train.txt"]
        self.neigh_filename = ["a_p_train.txt", "v_p_train.txt"]
        
        self.p_title_embed = torch.zeros(args.P_n, args.embed_dim, dtype=torch.float32)
        self.p_abstract_embed = torch.zeros(args.P_n, args.embed_dim, dtype=torch.float32)
        
        self.p_net_embed = torch.zeros(args.P_n, args.embed_dim, dtype=torch.float32)
        self.a_net_embed = torch.zeros(args.A_n, args.embed_dim, dtype=torch.float32)
        self.v_net_embed = torch.zeros(args.V_n, args.embed_dim, dtype=torch.float32) 
        
        self.p_a_net_embed = torch.empty(0)
        self.p_v_net_embed = torch.empty(0)
        self.p_p_net_embed = torch.empty(0)
        
        self.a_text_embed = torch.zeros(args.A_n, args.embed_dim, dtype=torch.float32)
        self.v_text_embed = torch.zeros(args.V_n, args.embed_dim, dtype=torch.float32)
    ...
    def read_content_file(self): # p_text, p_abstract, p_net, a_net, v_net
        ...
        for f_name in self.content_filename:
            with open("../../data/aminer" + self.args.data_path + f_name, 'r') as file:            
    ...
    def read_walk_file(self): # all edges indices based on neighbours in random walks
        ...
        with open("../../data/aminer_edge/het_neigh_train.txt", 'r') as file:            
            lines = file.readlines()
    ...
    def read_label_file(self):
        ...
        with open("../../data/aminer" + self.args.data_path + "a_class_train.txt", 'r') as file:            
            lines = file.readlines()
        for i, line in enumerate(lines):
                entries =  line.strip().split(',')

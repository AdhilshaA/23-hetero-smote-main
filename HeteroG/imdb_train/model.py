...
class Het_Agg(nn.Module):
    ...
    def forward(self, x_list, edges_list, x_node, num_node):
        m_aggr = self.m_aggregate(x_list[0], edges_list[0], num_node)
        a_aggr = self.a_aggregate(x_list[1], edges_list[1], num_node)
        d_aggr = self.d_aggregate(x_list[2], edges_list[2], num_node)
...
class Het_NetEn(nn.Module):
    ...
    def forward(self, x_list, data):
        ...
        x_list[0] = self.m_het(x_list, m_edges_list, x_list[0], x_list[0].size(0)) 
        x_list[1] = self.a_het(x_list, a_edges_list, x_list[1], x_list[1].size(0))
        x_list[2] = self.d_het(x_list, d_edges_list, x_list[2], x_list[2].size(0))
...
#             x_list = [self.m_cont, self.a_cont, self.d_cont]

#             self.m_cont = self.m_het(x_list, m_edges_list, self.m_cont, self.m_cont.size(0)) 
#             self.a_cont = self.a_het(x_list, a_edges_list, self.a_cont, self.a_cont.size(0))
#             self.d_cont = self.d_het(x_list, d_edges_list, self.d_cont, self.d_cont.size(0))
...
#         x = self.m_het(x_list, edge_list, x_list[0], x_list[0].size(0))
#         x = torch.relu(x)
#         x = F.dropout(x, self.dropout, training=self.training)
#         x = self.mlp(x)

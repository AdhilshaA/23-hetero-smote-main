...
def train_smote(data, edge_indices, encoder, classifier, decoder_list, train_idx, val_idx, test_idx, args, os_mode, train_mode):
    ...
    for epoch in range(args.num_epochs):
        ...
        evaluate_class_performance(outputs[new_train_idx].clone().detach(), new_labels[new_train_idx].clone().detach(), args)
        ...
        if epoch in epochs:
            ...
            if val_idx is not None:
                ac, auc, f1, auc_scr, f1_scr = test_smote(data, edge_indices, encoder, classifier, decoder_list, val_idx, args = args,
            ...
            if test_idx is not None:
                ac, auc, f1, auc_scr, f1_scr = test_smote(data, edge_indices, encoder, classifier, decoder_list, test_idx, args = args,
...
# Test function
def test_smote(data, edge_indices, encoder, classifier, decoder_list, test_idx, os_mode, train_mode, args, dataset = "Test"):
    encoder.eval()
    classifier.eval()
    ...
    with torch.no_grad():
        ...
        auc, f1, auc_list, f1_list = evaluate_class_performance(outputs[test_idx].clone().detach(), data['m'].y[test_idx].clone().detach(), args)

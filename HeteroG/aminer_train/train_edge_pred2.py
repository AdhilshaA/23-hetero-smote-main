...
from metric2 import edge_loss, accuracy, evaluate_class_performance
...
def train_smote(data, edge_indices, encoder, encoder2, classifier, decoder_list, decoder_list2, train_idx, val_idx, test_idx, args, os_mode, train_mode):
    ...
    for epoch in range(args.num_epochs):
        ...
        if os_mode == 'gsm':
            ...
            edge_ac, loss_de, new_edge_indices = het_decode(decoder_list, x_list, edge_indices, ar_edge_indices, args, 
                                                            train_mode, dataset = 'Train', x = x_list[0], k =0)
            
            
            edge_ac2, loss_de2, new_p_edges_list = het_decode(decoder_list2, x_list, p_edges_list, 
            args = args, train_mode = train_mode, ar_edge_indices = p_edges_list, dataset = 'Train', x = x_list[1], k =1)
            # print(new_edge_indices[0][0].max().item(), new_edge_indices[1][0].max().item(), new_edge_indices[2][0].max().item())
            loss_de = [loss_de[0]*w1, loss_de[1]*w2, loss_de[2]*w3]
            loss_de2 = [loss_de2[0]*w1, loss_de2[1]*w2, loss_de2[2]*w3]
            # print(loss_de)
            del ar_edge_indices
        ...
        x_list = encoder2(x_list, new_p_edges_list)
        ...
        if train_mode == 'preO' or train_mode == 'preT' or train_mode == 'pret'  or train_mode == 'preo': 
            loss = sum(loss_de) * args.de_weight + sum(loss_de2) * args.de_weight + loss_cls
            # print(loss_cls.item(), loss.item())
        elif train_mode == 'recon':
            loss = sum(loss_de) * args.de_weight * 10 +  sum(loss_de2) * args.de_weight * 10
        else:
            loss = loss_cls
        ...
        if train_mode != 'newG':
            ...
            if train_mode == 'preO' or train_mode == 'preT' or train_mode == 'recon' or train_mode == 'pret'  or train_mode == 'preo':
                for optimizer in optimizer_de:
                    optimizer.step()
                for optimizer in optimizer_de2:
                    optimizer.step()
        ...
        if epoch in epochs:      
            ...
            print(f'Epoch [{epoch + 1}/{args.num_epochs}], Loss: {loss.item():.4f}, Accuracy: {acc:.4f}, Edge Accuracy: {edge_ac, edge_ac2}')
        ...
        if epoch in epochs:
            ...
            if val_idx is not None:
                ac, auc, f1, auc_scr, f1_scr = test_smote(data, edge_indices, encoder, encoder2, classifier, decoder_list, decoder_list2, 
                val_idx, args = args, dataset='Validation', os_mode='no', train_mode=train_mode)  # Evaluate on the validation dataset
            ...
            if test_idx is not None:
                ac, auc, f1, auc_scr, f1_scr = test_smote(data, edge_indices, encoder, encoder2, classifier, decoder_list, decoder_list2,
                test_idx, args = args, dataset='Test', os_mode='no', train_mode=train_mode)  # Evaluate on the test dataset
...
# Test function
def test_smote(data, edge_indices, encoder, encoder2, classifier, decoder_list, decoder_list2, test_idx, os_mode, train_mode, args, dataset = "Test"):
    encoder.eval()
    classifier.eval()
    for decoder in decoder_list: decoder.eval()
    for decoder in decoder_list2: decoder.eval()
    loss_de, loss_de2, edge_ac, edge_ac2 = [], [], [], []
    auc_list, f1_list = [], []
    p_edges_list = [data['p', 'walk', 'a'].edge_index, data['p', 'walk', 'p'].edge_index, data['p', 'walk', 'v'].edge_index]
    ...
    with torch.no_grad():
        ...
        if os_mode == 'gsm':
            edge_ac, loss_de, new_edge_indices = het_decode(decoder_list, x_list, edge_indices, 
                ar_edge_indices = None, args = args, train_mode = train_mode, dataset = dataset, x = x_list[0], k =0)
        ...
        x_list = encoder2(x_list, p_edges_list)
        outputs = classifier(x_list, new_edge_indices)
        ...
        if train_mode == 'preO' or train_mode == 'preT' or train_mode == 'pret'  or train_mode == 'preo': 
            ...
            loss = torch.tensor(sum(loss_de))  + torch.tensor(sum(loss_de2))  
        else:
            loss = loss_cls            
        
        # print(loss)
        acc = accuracy(outputs[test_idx], data['a'].y[test_idx]) 
        print(f'{dataset} Loss: {loss.item():.4f}, {dataset} Accuracy: {acc:.4f}, {dataset} Edge Accuracy: {edge_ac, edge_ac2}')
        auc, f1, auc_list, f1_list = evaluate_class_performance(outputs[test_idx].cpu(), data['a'].y[test_idx].cpu(), args)
...
def het_decode(decoder_list, x_list, edge_indices, ar_edge_indices, args, train_mode, dataset, x, k):
    edge_ac = []
    loss_de = []
    edge_list = []
    acc = 0
    ...
    for i in range(len(decoder_list)):
        ...
        adj_new = decoder_list[i](x, x_list[i])
        ori_row_num = args.node_dim[k]
        ori_col_num = args.node_dim[i]
        # print(i, ori_row_num, ori_col_num)
        
        adj_old = torch.zeros((ori_row_num, ori_col_num), dtype = torch.float32, device=x.device)
        adj_old[edge_indices[i][0], edge_indices[i][1]] = 1.0
        
        if ar_edge_indices is not None:
            adj_ar = torch.zeros((x.size(0), x_list[i].size(0)), dtype = torch.float32, device=x.device)

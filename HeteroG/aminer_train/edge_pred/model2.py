...
class Het_Agg(nn.Module):
    ...
    def forward(self, x_list, edges_list, x_node, num_node):
        a_aggr = self.a_aggregate(x_list[0], edges_list[0], num_node)
        p_aggr = self.p_aggregate(x_list[1], edges_list[1], num_node)
        v_aggr = self.v_aggregate(x_list[2], edges_list[2], num_node)
...
class Het_En(nn.Module):
    def __init__(self, embed_dim, dropout):
        ...
        self.p_con = Content_Agg(embed_dim, 5, dropout)
    ...
    def forward(self, data, os_mode):
        
        a_embed_list = [data['a_net_embed'].x, data['a_text_embed'].x]
        p_embed_list = [data['p_title_embed'].x, data['p_abstract_embed'].x, data['p_net_embed'].x, data['p_a_net_embed'].x, data['p_v_net_embed'].x]
        ...
        if os_mode != 'up' or os_mode != 'smote':
            ...
            self.a_cont = self.a_het(x_list, a_edges_list, self.a_cont, self.a_cont.size(0))
            self.p_cont = self.p_het(x_list, p_edges_list, self.p_cont, self.p_cont.size(0))
            self.v_cont = self.v_het(x_list, v_edges_list, self.v_cont, self.v_cont.size(0))
...
class Het_classify(nn.Module):
    ...
    def forward(self, x_list, edge_list):
        x = self.a_het(x_list, edge_list, x_list[0], x_list[0].size(0))
        x = torch.relu(x)
        x = F.dropout(x, self.dropout, training=self.training)
        x = self.mlp(x)
...
class Edge_classify(nn.Module):
    def __init__(self, embed_dim, nclass, dropout):
        ...
        self.mlp = nn.Sequential(nn.Sigmoid(), nn.Linear(embed_dim, 64),nn.Linear(64, nclass))
        self.dropout = dropout
        self.reset_parameters()
    ...
    def forward(self, x_list, edge_list, target_edge, x, os_mode):
        if os_mode != 'edge_sm':
            features = self.p_het(x_list, edge_list, x_list[1], x_list[1].size(0))
            features = torch.relu(self.lin(features))
            x = features[target_edge[:, 0]] * features[target_edge[:, 1]] 
        x = self.mlp(x)
        x = F.dropout(x, self.dropout, training=self.training)
